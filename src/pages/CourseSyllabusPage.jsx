...
export default function CourseSyllabusPage() {
  ...
  const save = async () => {
    try {
      ...
      const payload = {
        ...values,
        course_name: values.vietnamese_title || values.course_name || '',
        course_name_en: values.english_title || values.course_name_en || '',
      }
      const saved = await saveSyllabusBundle(payload, { instructors, objectives, clos, mappings, teachingPlans, assessments, references })
      setBundle(saved)
      setOpen(false)
